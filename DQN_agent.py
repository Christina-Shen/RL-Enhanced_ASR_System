...
class DQNAgent:
    def __init__(self, state_size, action_size,n_action):
        ...
        self.qnetwork_local = QNetwork(state_size, action_size,42)
        self.qnetwork_target = QNetwork(state_size, action_size,42)
    ...
    def act(self, state, eps=0.):
        ...
        with torch.no_grad():
            action_values = self.qnetwork_local(state).squeeze(0)
        self.qnetwork_local.train()

        # action_values 应该是一维的，形状为 (action_size,)
        action_values_np = action_values.cpu().data.numpy()
        top_actions = action_values_np.argsort()[-self.n_action:] 

        # Epsilon-greedy action selection
        if random.random() > eps:
            return top_actions
        else:
            return np.random.choice(self.action_size, self.n_action, replace=False)
